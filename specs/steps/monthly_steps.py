...
def step_monthly_record_exists_generic(context):
    ...
    company_resp = requests.post(f"{API_BASE_URL}/companies", json=company_data, headers=auth)
    if company_resp.status_code in [200, 201]:
        ...
        resp = requests.post(f"{API_BASE_URL}/monthly", json=record_data, headers=auth)
...
def step_create_monthly_record(context, company_id, mes_ano):
    ...
    if context.monthly_company_uuid:
        ...
        resp = requests.post(f"{API_BASE_URL}/monthly", json=create_data, headers=get_auth_headers(context))
        if resp.status_code in [200, 201]:
            context.target_monthly_uuid = resp.json().get("id")
            context.target_monthly = resp.json()
...
def step_monthly_record_exists(context, company_id, mes_ano):
    ...
    if response.status_code == 200:
        ...
        if context.monthly_company_uuid:
            ...
            resp = requests.post(f"{API_BASE_URL}/monthly", json=create_data, headers=auth)
            if resp.status_code in [200, 201]:
                context.target_monthly_uuid = resp.json().get("id")
                context.target_monthly = resp.json()
        else:
...
def step_save_monthly_record(context):
    ...
    if 'company_id' not in context.monthly_form_data:
        ...
        comp_resp = requests.post(f"{API_BASE_URL}/companies", json=company_data, headers=headers)
...
def step_retrieve_company_monthly(context, company_id):
    ...
    if company_uuid:
        response = requests.get(f"{API_BASE_URL}/companies/{company_uuid}/monthly", headers=auth)
        if response.status_code == 200:
...
def step_filter_monthly(context, mes_ano):
    response = requests.get(f"{API_BASE_URL}/monthly", params={"mes_ano": mes_ano}, headers=get_auth_headers(context))
    if response.status_code == 200:
        context.monthly_records = response.json()
        context.last_response_status = response.status_code
...
def step_retrieve_all_monthly(context):
    response = requests.get(f"{API_BASE_URL}/monthly", headers=get_auth_headers(context))
    if response.status_code == 200:
        context.monthly_records = response.json()
        context.last_response_status = response.status_code
    else:
...
def step_try_create_monthly(context):
    ...
    company_uuid = getattr(context, 'monthly_company_uuid', None)
    if not company_uuid:
        resp = requests.get(f"{API_BASE_URL}/companies", headers=auth)
    ...
    if company_uuid:
        ...
        response = requests.post(f"{API_BASE_URL}/monthly", json=context.monthly_form_data, headers=auth)
        context.last_response = response
        context.last_response_status = response.status_code
    else:
        context.last_response_status = 404
...
def step_try_delete_monthly(context):
    ...
    uuid = getattr(context, 'target_monthly_uuid', None)
    if uuid:
        response = requests.delete(f"{API_BASE_URL}/monthly/{uuid}", headers=get_auth_headers(context))
        context.last_response = response
        context.last_response_status = response.status_code
    else:
        context.last_response_status = 404
...
def step_monthly_404(context):
    uuid = getattr(context, 'target_monthly_uuid', None)
    if uuid:
        response = requests.get(f"{API_BASE_URL}/monthly/{uuid}", headers=get_auth_headers(context))
...
def step_retrieve_both_same_month(context):
    company_uuid = getattr(context, 'monthly_company_uuid', None)
    if company_uuid:
        response = requests.get(f"{API_BASE_URL}/companies/{company_uuid}/monthly", headers=get_auth_headers(context))
        if response.status_code == 200:
