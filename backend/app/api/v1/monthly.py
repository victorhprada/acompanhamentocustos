...
def get_future_months(mes_ano: str) -> list[str]:
    """Get all months after mes_ano until end of 2026."""
    try:
...
def list_monthly_records(
    ...
    query = query.order("mes_ano", desc=False)
...
def create_monthly_record(
    ...
    propagate: bool = Query(True, description="Propagate values to future months"),
    supabase: Client = Depends(get_supabase)
):
    # Check for duplicate
    existing = (
    ...
    if existing.data:
        raise HTTPException(
            status_code=400,
            detail="Record already exists for this month and product"
        )

    result = supabase.table("monthly_records").insert(record.model_dump(mode='json')).execute()
    if not result.data:
        raise HTTPException(status_code=400, detail="Failed to create monthly record")
    
    created = result.data[0]
    
    # Propagate to future months if requested
    if propagate:
        future_months = get_future_months(record.mes_ano.isoformat())
        for future_month in future_months:
            # Check if record already exists for this future month
            check = (
                supabase.table("monthly_records")
                .select("id")
                .eq("company_id", record.company_id)
            ...
            if not check.data:
                ...
                inherited_data = record.model_dump(mode='json')
                inherited_data["mes_ano"] = future_month
                supabase.table("monthly_records").insert(inherited_data).execute()
        
        # Mark the original as the source (add metadata via update)
        supabase.table("monthly_records").update(
            {"_source": True}
...
def update_monthly_record(
    ...
    propagate: bool = Query(True, description="Propagate changes to future months"),
    supabase: Client = Depends(get_supabase)
):
    ...
    if propagate and update_data:
        future_months = get_future_months(current["mes_ano"])
        for future_month in future_months:
            # Find existing record for this future month
            check = (
                supabase.table("monthly_records")
                .select("*")
                .eq("company_id", current["company_id"])
                .eq("produto", current["produto"])
                .eq("mes_ano", future_month)
                .execute()
            )
            if check.data:
                # Update existing future record with new values
                supabase.table("monthly_records").update(update_data).eq("id", check.data[0]["id"]).execute()
            else:
                # Create new inherited record - copy current values + updates
...
def delete_monthly_record(record_id: str, supabase: Client = Depends(get_supabase)):
    existing = supabase.table("monthly_records").select("*").eq("id", record_id).execute()
    if not existing.data:
        raise HTTPException(status_code=404, detail="Monthly record not found")
